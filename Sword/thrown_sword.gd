...
@onready var collision_body : CollisionShape2D = $"SwordBody/CollisionHitbox"
@onready var pickup_zone : Area2D = $"SwordBody/PickupZone"
@onready var hitbox : CollisionShape2D = $"SwordBody/DamageZone/DamageHitbox"
@onready var sword_animation : AnimationPlayer = $"Spin"
@onready var retrieval_line : Line2D = $"RetrievalLine"
@onready var thrown_sprite : Sprite2D = $"SwordBody/ThrownSprite"
@onready var stuck_sprite : AnimatedSprite2D = $"SwordBody/StuckSprite"

@onready var trajectory_line : Line2D = $"TrajectoryLine"
@onready var trajectory_body : RigidBody2D = $"TrajectoryLine/TrajectoryBody"
@onready var trajectory_hitbox: CollisionShape2D = $"TrajectoryLine/TrajectoryBody/TrajectoryHitbox"
...
func _physics_process(delta):
	...
	if player.sword_held == true:
		...
		trajectory_body.position = player.position
	
	check_bounce()

	if Input.is_action_pressed("attack") and player.sword_held:
		player.charge_time += delta
		# trajectory_line.sword_body.show()
		# update_trajectory(final_throw_velocity, delta)
	else:
		# trajectory_line.sword_body.hide()
		pass
